...
// TODO: dispatchsvc 尚未提供 RejectDispatch，当前仅完成鉴权与请求校验，不实际调用下游。
...
// TODO: dispatchsvc 尚未支持按 driver_id 查询，当前仅完成鉴权与分页参数解析，不实际调用下游。
func ListMyDispatchesHandler(svcCtx *svc.ServiceContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		resp, err := logic.NewOrderLogic(r.Context(), svcCtx).ListMyDispatches(int64(claims.AccountID), req.Page, req.PageSize)
		if err != nil {
			writeParamError(w, err)
			return
		}
