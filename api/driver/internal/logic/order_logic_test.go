...
func TestRejectOrderWaitsForDispatchRPC(t *testing.T) {
	...
	if err != ErrDispatchDependencyNotReady {
		t.Fatalf("RejectOrder() error = %v, want %v", err, ErrDispatchDependencyNotReady)
	}
}

func TestListMyDispatchesWaitsForDriverQueryRPC(t *testing.T) {
	logic := NewOrderLogic(context.Background(), &svc.ServiceContext{})

	_, err := logic.ListMyDispatches(25, 0, 1000)
	if err != ErrDispatchDependencyNotReady {
		t.Fatalf("ListMyDispatches() error = %v, want %v", err, ErrDispatchDependencyNotReady)
	}
}
