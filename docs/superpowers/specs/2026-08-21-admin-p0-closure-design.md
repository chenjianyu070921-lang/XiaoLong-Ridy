...
迁移目标为远程 `xiaolong_ridy` 数据库。执行前先使用只读查询核验 `07_admin_operation_business.sql` 定义的七张表是否存在；仅在缺失时执行该脚本。脚本只包含 `CREATE TABLE IF NOT EXISTS`，不修改已有表或业务数据。执行后核验表、索引和最小查询，并记录执行结果。
