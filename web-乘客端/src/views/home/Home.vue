...
async function initMap() {
  ...
  try {
    ...
    placeSearch.value = new AMap.PlaceSearch({ pageSize: 20, pageIndex: 1, extensions: 'base' })
...
function reverseGeocode(lng, lat) {
  return new Promise(resolve => {
    if (!geocoder.value) return resolve('当前位置')
    geocoder.value.getAddress([Number(lng), Number(lat)], (status, result) => {
      if (status !== 'complete' || !result?.regeocode) return resolve('当前位置')
      resolve(result.regeocode.formattedAddress || formatDetailedAddress(result.regeocode.addressComponent))
    })
  })
}
...
async function applyCurrentLocation({ lng, lat, address, name }, useAsPickup = true) {
  ...
  if (!text || ['当前位置', '我的位置', '当前城市'].includes(text)) {
    text = await reverseGeocode(normalizedLng, normalizedLat)
  }
  if (!text || ['当前位置', '我的位置', '当前城市'].includes(text)) {
  ...
  if (useAsPickup) {
    ...
    orderStore.setOrderParams({ fromLng: normalizedLng, fromLat: normalizedLat, fromAddress: pickupName })
  }
...
function locateByAMap() {
  return new Promise((resolve, reject) => geolocation.value?.getCurrentPosition((status, result) => status === 'complete' && result?.position ? resolve({ lng: result.position.lng, lat: result.position.lat, address: result.formattedAddress || formatDetailedAddress(result.addressComponent, '当前位置') }) : reject(result || new Error('高德定位失败'))))
}

function locateByBrowser() {
  return new Promise((resolve, reject) => {
    if (!navigator.geolocation) return reject(new Error('当前浏览器不支持定位'))
    navigator.geolocation.getCurrentPosition(async result => resolve({ lng: result.coords.longitude, lat: result.coords.latitude, address: await reverseGeocode(result.coords.longitude, result.coords.latitude) }), reject, { enableHighAccuracy: true, timeout: 12000, maximumAge: 15000 })
  })
}
...
function searchByKeyword(value) {
  ...
  placeSearch.value.search(value, (status, result) => handleSearchResult(sequence, status, result))
}
...
async function selectSearchResult(item) {
  if (searchMode.value === 'pickup') {
    await applyCurrentLocation({ lng: item.lng, lat: item.lat, name: item.name, address: item.displayAddress || item.address || item.name })
    closeLocationSearch()
    return
  }
  destinationAddress.value = item.name
  orderStore.setOrderParams({ toAddress: item.name, toLat: item.lat, toLng: item.lng })
...
async function loadActiveOrder() {
  try {
    const result = await getOrders({ page: 1, pageSize: 20, status: 0 })
