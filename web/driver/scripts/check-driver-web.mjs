...
assert(home.includes('@logout="logoutDriver"'), 'driver home mine panel must expose a logout action')
assert(minePanel.includes('退出登录'), 'driver mine panel must render a visible logout entry')
assert(minePanel.includes('class="logout-button"'), 'driver mine panel logout entry must have a dedicated visibility hook')
assert(functionSource(home, 'logoutDriver').includes('previewOrder.value = null'), 'driver logout must clear any preview order before leaving home')
assert(functionSource(login, 'submitLogin').includes('if (loading.value) return'), 'driver login submit must ignore duplicate clicks while loading')
