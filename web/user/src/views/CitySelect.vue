<template>
  <div class="city-page">
    ...
    <div class="current-city">当前城市：<strong>{{ selectedCity?.name || '定位中' }}</strong></div>
...
import { cities, readSelectedCity, saveSelectedCity } from '@/data/cities'
const router = useRouter(); const keyword = ref(''); const selectedCity = ref(readSelectedCity()); const letters = [...new Set(cities.map(city => city.initial))]
...
function selectCity(city) { localStorage.setItem('passenger-manual-city', '1'); saveSelectedCity(city); router.replace({ path: '/home', query: { city: city.adcode } }) }
