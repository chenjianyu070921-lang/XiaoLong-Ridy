...
const isReasonAction = computed(() => ['freeze', 'unfreeze', 'freezeDriver', 'unfreezeDriver', 'cancel', 'release', 'approveWithdraw', 'rejectWithdraw'].includes(props.type))
...
const copyRecord = () => {
  ...
  if (props.type === 'workOrderAction') {
    form.version = props.record?.version || 0
    // 页面传入的 allowed_actions 已按当前工单状态和管理员角色过滤。
    if (props.record?.allowed_actions?.length && !props.record.allowed_actions.includes(form.action)) form.action = props.record.allowed_actions[0]
  }
}
...
const toPayload = () => {
  ...
  if (props.type === 'freeze' || props.type === 'unfreeze' || props.type === 'freezeDriver' || props.type === 'unfreezeDriver') return { reason: form.reason, remark: form.remark }
  if (props.type === 'approveWithdraw' || props.type === 'rejectWithdraw') return { remark: form.remark }
...
<template>
  <el-dialog :model-value="modelValue" :title="title" width="680px" destroy-on-close @update:model-value="close">
    <el-form ref="formRef" :model="form" :rules="rules" label-width="108px" class="business-form">
      ...
      <template v-else-if="props.type === 'workOrderAction'"><el-form-item label="流转动作"><el-select v-model="form.action"><el-option v-for="item in [['assign','分配'],['follow','跟进'],['arbitrate','仲裁'],['close','结案'],['reopen','重新打开']].filter((item) => !props.record?.allowed_actions?.length || props.record.allowed_actions.includes(item[0]))" :key="item[0]" :label="item[1]" :value="item[0]"/></el-select></el-form-item><el-form-item label="处理人 ID"><el-input v-model="form.assignee_id" /></el-form-item><el-form-item label="处理内容" prop="content"><el-input v-model="form.content" type="textarea" /></el-form-item><el-form-item label="仲裁结论"><el-input v-model="form.arbitration_result" type="textarea" /></el-form-item></template>
      ...
      <template v-else-if="props.type === 'riskHitAction'"><el-form-item label="命中数量"><el-input :model-value="`${form.ids?.length || 0} 条`" disabled /></el-form-item><el-form-item label="处置动作"><el-select v-model="form.risk_action"><el-option label="复核通过" value="review_pass"/><el-option label="加入黑名单" value="add_blacklist"/><el-option label="冻结用户或司机" value="freeze"/><el-option label="转工单" value="create_work_order"/></el-select></el-form-item><el-form-item v-if="form.risk_action === 'create_work_order'" label="工单标题"><el-input v-model="form.work_order_title" /></el-form-item><el-form-item v-if="form.risk_action === 'create_work_order'" label="优先级"><el-select v-model="form.priority"><el-option label="低" :value="1"/><el-option label="中" :value="2"/><el-option label="高" :value="3"/><el-option label="紧急" :value="4"/></el-select></el-form-item><el-form-item label="处置说明" prop="reason"><el-input v-model="form.reason" type="textarea" /></el-form-item></template>
      ...
      <template v-else-if="['approveWithdraw','rejectWithdraw'].includes(props.type)"><el-form-item label="审核备注" :required="props.type==='rejectWithdraw'"><el-input v-model="form.remark" type="textarea" :placeholder="props.type==='approveWithdraw' ? '打款成功备注（可选）' : '打款失败原因（必填）'" /></el-form-item></template>
      <template v-else-if="isReasonAction"><el-form-item label="原因" prop="reason"><el-input v-model="form.reason" type="textarea" /></el-form-item><el-form-item v-if="props.type !== 'release' && props.type !== 'unfreezeDriver'" label="备注"><el-input v-model="form.remark" type="textarea" /></el-form-item></template>
