...
import { ordersApi, marketingApi, statisticsApi, riskApi, logsApi, refundRetryApi } from '../../api/modules'
...
const dashboard = ref({ overview: {}, orders: {}, coupons: {}, drivers: {}, revenue: {} });
...
const loadDashboard = async () => { loading.value = true; try { const params = statisticsParams(); const [overview, orders, drivers, revenue, coupons] = await Promise.all([statisticsApi.overview(params), statisticsApi.orders(params), statisticsApi.drivers(params), statisticsApi.revenue(params), statisticsApi.coupons(params)]); dashboard.value = { overview: overview || {}, orders: orders || {}, drivers: drivers || {}, revenue: revenue || {}, coupons: coupons || {} } } finally { loading.value = false } }
onMounted(async () => { if (kind.value === 'dashboard' || isStatistics.value) return loadDashboard(); if (isDetail.value) { await loadDetail(); return }; await load() }); watch(() => route.path, async () => { detail.value = null; selectionRows.value = []; page.value = 1; if (kind.value === 'dashboard' || isStatistics.value) return loadDashboard(); if (isDetail.value) { await loadDetail(); return }; await load() });
</script>
...
@media(max-width:1000px){.metric-grid,.detail-grid{grid-template-columns:1fr}.panel-grid{grid-template-columns:1fr}.filters,.statistics-actions{flex-wrap:wrap}}
</style>
