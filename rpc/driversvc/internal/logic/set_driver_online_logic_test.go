...
func TestSetDriverOnlineAllowsWithoutCertification(t *testing.T) {
	mr := miniredis.RunT(t)
	rdb := redis.NewClient(&redis.Options{Addr: mr.Addr()})
	...
	if _, err := logic.SetDriverOnline(&proto.SetDriverOnlineRequest{
		...
		t.Fatalf("SetDriverOnline() should allow driver without certification, got err = %v", err)
	}
	online, err := rdb.SIsMember(context.Background(), constants.RedisDriverOnline, "25").Result()
	if err != nil {
		t.Fatalf("SIsMember(online) error = %v", err)
...
func TestSetDriverOnlineAllowsWithoutVehicle(t *testing.T) {
	mr := miniredis.RunT(t)
	rdb := redis.NewClient(&redis.Options{Addr: mr.Addr()})
	...
	if _, err := logic.SetDriverOnline(&proto.SetDriverOnlineRequest{
		...
		t.Fatalf("SetDriverOnline() should allow driver without vehicle, got err = %v", err)
	}
	online, err := rdb.SIsMember(context.Background(), constants.RedisDriverOnline, "25").Result()
	if err != nil {
		t.Fatalf("SIsMember(online) error = %v", err)
