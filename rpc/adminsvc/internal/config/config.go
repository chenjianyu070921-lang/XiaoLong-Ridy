...
type Config struct {
	...
	// PaysRPC 供后台订单详情查询支付与结算记录，数据读取由 paysvc 负责。
	PaysRPC zrpc.RpcClientConf `json:"PaysRPC,optional" yaml:"PaysRPC"`
	...
	// AiAgent 是 AI 运营助手配置。DemoEnabled 生产必须为 false。
	AiAgent AiAgentConfig `json:"AiAgent,optional" yaml:"AiAgent"`
}

// AiAgentConfig 定义 AI 运营助手配置。
type AiAgentConfig struct {
	// DemoEnabled 是否允许演示快照；生产必须为 false。
	DemoEnabled bool `json:"DemoEnabled,optional" yaml:"DemoEnabled"`
	// Model 是外部 LLM 配置，密钥从环境变量读取，绝不写入配置。
	Model AiModelConfig `json:"Model,optional" yaml:"Model"`
	// Conversation 会话上下文管理参数。
	Conversation AiConversationConfig `json:"Conversation,optional" yaml:"Conversation"`
}

// AiModelConfig 定义外部 LLM 连接参数。
type AiModelConfig struct {
	Endpoint       string `json:"Endpoint,optional" yaml:"Endpoint"`
	Name           string `json:"Name,optional" yaml:"Name"`
	TimeoutSeconds int    `json:"TimeoutSeconds,optional" yaml:"TimeoutSeconds"`
}

// AiConversationConfig 定义会话上下文管理参数。
type AiConversationConfig struct {
	TTLSeconds int    `json:"TTLSeconds,optional" yaml:"TTLSeconds"`
	MaxRounds  int    `json:"MaxRounds,optional" yaml:"MaxRounds"`
	KeyPrefix  string `json:"KeyPrefix,optional" yaml:"KeyPrefix"`
}
